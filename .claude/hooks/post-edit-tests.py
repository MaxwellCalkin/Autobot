...
def has_python_tests(project_dir: Path) -> bool:
    """Check if the project has Python test files."""
...
def detect_test_command(project_dir: Path) -> list[str] | None:
    ...
    # Python - only if there are actual test files
    if (project_dir / 'pyproject.toml').exists() or (project_dir / 'setup.py').exists():
        if has_python_tests(project_dir):
